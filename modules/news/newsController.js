  ...
  try {
    ...
    for (let i of list) {
      const one = new newsSchema({
        ...
        title: i.title,
        post_url: i.link
          .replace("https://vnexpress.net", "")
          .replace(".html", ""),
        title_search: i.link
          .replace("https://vnexpress.net/", "")
          .replace(".html", ""),
        news_id: i.news_id,
        publishedAt: moment(i.publishedAt, "DD/MM/YYYY, HH:mm"),
  ...
  try {
    ...
    for (let i of list) {
      if (i.link) {
        ...
        let category = "";
        $(
          "section.section.page-detail.top-detail > div.container > div.sidebar-1 > div.header-content.width_common > span.date"
        ).each((index, testEl) => {
          publishedAt = $(testEl).text();
          const startPos = publishedAt.indexOf(", ");
          const endPos = publishedAt.indexOf(" (");
        ...
        $(
          "section.section.page-detail.top-detail > div.container > div.sidebar-1 > article.fck_detail"
        ).each((index, testEl) => {
          data = $(testEl).html();
          const cutPoint = data.indexOf(
...
newsController.LishLink = async (page) => {
  try {
    const listAllNews = await newsSchema
      .find()
      .select("news_id")
      .sort({ publishedAt: -1 });
    ...
    let links = [];
    $(
      "div.width_common.list-news-subfolder > article.item-news.item-news-common"
    ).each((index, item) => {
      let itemAdd = {};
      $(item)
        .find(".title-news > a")
